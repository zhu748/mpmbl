...
func TestHandleClaudeStreamRealtimeAdaptsDMSLToolNameToClaudeTool(t *testing.T) {
	h := &Handler{}
	payload := `<|DMSL|tool_calls><|DMSL|invoke name="write-file"><|DMSL|parameter name="path">a.txt</|DMSL|parameter></|DMSL|invoke></|DMSL|tool_calls>`
	resp := makeClaudeSSEHTTPResponse(
	...
	for _, f := range findClaudeFrames(frames, "content_block_start") {
		contentBlock, _ := f.Payload["content_block"].(map[string]any)
		if contentBlock["type"] == "tool_use" {
			if contentBlock["name"] != "Write_File" {
				t.Fatalf("expected canonical Claude tool name Write_File, got %#v body=%s", contentBlock["name"], rec.Body.String())
			}
			return
		}
	}
	t.Fatalf("expected DMSL tool_use block, body=%s", rec.Body.String())
}

func TestHandleClaudeStreamRealtimeRejectsUnknownDMSLTool(t *testing.T) {
	h := &Handler{}
	payload := `<dmsl tool_calls><dmsl invoke name="DeleteAll"><dmsl parameter name="path">/tmp</dmsl parameter></dmsl invoke></dmsl tool_calls>`
	resp := makeClaudeSSEHTTPResponse(
		`data: {"p":"response/content","v":"`+strings.ReplaceAll(payload, `"`, `\"`)+`"}`,
		`data: [DONE]`,
	)
	...
	h.handleClaudeStreamRealtime(rec, req, resp, "claude-sonnet-4-5", []any{map[string]any{"role": "user", "content": "use tool"}}, false, false, []string{"ReadFile"}, nil)

	frames := parseClaudeFrames(t, rec.Body.String())
	for _, f := range findClaudeFrames(frames, "content_block_start") {
		contentBlock, _ := f.Payload["content_block"].(map[string]any)
		if contentBlock["type"] == "tool_use" {
			t.Fatalf("did not expect unknown DMSL tool to become tool_use, body=%s", rec.Body.String())
		}
	}
}
