...
func TestParseToolCallsToleratesDSMLVariantTags(t *testing.T) {
	tests := []struct {
		name string
		open string
		invo string
		...
		{name: "hyphenated tool calls", open: "<dsml-tool-calls>", invo: "<dsml-invoke name=\"Read\">", parm: "<dsml-parameter name=\"file_path\">/tmp/hyphen.txt</dsml-parameter>", clos: "</dsml-invoke></dsml-tool-calls>"},
		{name: "underscored dsml prefix", open: "<dsml_tool_calls>", invo: "<dsml_invoke name=\"Read\">", parm: "<dsml_parameter name=\"file_path\">/tmp/underscore.txt</dsml_parameter>", clos: "</dsml_invoke></dsml_tool_calls>"},
		{name: "backtick dsml prefix", open: "<```dsml tool_calls>", invo: "<```dsml invoke name=\"Read\">", parm: "<```dsml parameter name=\"file_path\">/tmp/backtick.txt</```dsml parameter>", clos: "</```dsml invoke></```dsml tool_calls>"},
		{name: "hash pipe separator", open: "<|#tool_calls>", invo: "<|#invoke name=\"Read\">", parm: "<|#parameter name=\"file_path\">/tmp/hash.txt</|#parameter>", clos: "</|#invoke></|#tool_calls>"},
		{name: "star pipe separator", open: "<|*tool_calls>", invo: "<|*invoke name=\"Read\">", parm: "<|*parameter name=\"file_path\">/tmp/star.txt</|*parameter>", clos: "</|*invoke></|*tool_calls>"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			text := strings.Join([]string{tt.open, tt.invo, tt.parm, tt.clos}, "\n")
			calls := ParseToolCalls(text, []string{"Read"})
			if len(calls) != 1 {
				t.Fatalf("expected one call from %s, got %#v", tt.name, calls)
			}
			if calls[0].Name != "Read" {
				t.Fatalf("expected Read call, got %#v", calls[0])
			}
			if got, _ := calls[0].Input["file_path"].(string); !strings.HasPrefix(got, "/tmp/") {
...
func TestParseToolCallsRejectsDSMLVariantLookalikes(t *testing.T) {
	tests := []string{
		`<dsml-tool-calls-extra><dsml-invoke name="Read"><dsml-parameter name="file_path">x</dsml-parameter></dsml-invoke></dsml-tool-calls-extra>`,
		`<dsml_tool_calls_extra><dsml_invoke name="Read"><dsml_parameter name="file_path">x</dsml_parameter></dsml_invoke></dsml_tool_calls_extra>`,
		`<|#tool_calls_extra><|#invoke name="Read"><|#parameter name="file_path">x</|#parameter></|#invoke></|#tool_calls_extra>`,
		`<|*tool_calls_extra><|*invoke name="Read"><|*parameter name="file_path">x</|*parameter></|*invoke></|*tool_calls_extra>`,
	}
	for _, text := range tests {
		if calls := ParseToolCalls(text, []string{"Read"}); len(calls) != 0 {
			t.Fatalf("expected no calls from lookalike tag, got %#v", calls)
		}
	}
}

func TestParseToolCallsDoesNotTreatBacktickFenceAsToolSyntax(t *testing.T) {
	text := "```dsml tool_calls>\n<```dsml invoke name=\"Read\"><```dsml parameter name=\"file_path\">x</```dsml parameter></```dsml invoke>\n</```dsml tool_calls>"
	res := ParseToolCallsDetailed(text, []string{"Read"})
	if len(res.Calls) != 0 {
		t.Fatalf("expected no calls from fenced-looking text without tag opener, got %#v", res.Calls)
	}
}
