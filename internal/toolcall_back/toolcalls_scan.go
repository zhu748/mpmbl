...
func scanToolMarkupTagAt(text string, start int) (ToolMarkupTag, bool) {
	...
	if next, ok := consumeToolMarkupNamePrefix(lower, text, i); ok {
		dsmlLike = true
		i = next
		for next, ok := consumeToolMarkupSeparator(text, i); ok; next, ok = consumeToolMarkupSeparator(text, i) {
			i = next
...
func consumeToolMarkupNamePrefix(lower, text string, idx int) (int, bool) {
	if next, ok := consumeToolMarkupNamePrefixOnce(lower, text, idx); ok {
		return next, true
	}
	if next, ok := consumeToolMarkupBacktickPrefix(lower, text, idx); ok {
		return next, true
	}
	return idx, false
}
...
func consumeToolMarkupNamePrefixOnce(lower, text string, idx int) (int, bool) {
	for _, prefix := range []string{"dsml", "dmsl"} {
		if !strings.HasPrefix(lower[idx:], prefix) {
			continue
		}
		next := idx + len(prefix)
		if next >= len(text) {
...
func consumeToolMarkupBacktickPrefix(lower, text string, idx int) (int, bool) {
	if idx >= len(text) || text[idx] != '`' {
		return idx, false
	}
	for next := idx; next < len(text) && text[next] == '`'; next++ {
		afterTicks := next + 1
		if prefixEnd, ok := consumeToolMarkupNamePrefixOnce(lower, text, afterTicks); ok {
...
func matchToolMarkupName(lower string, start int) (string, int) {
	for _, name := range toolMarkupNames {
		...
		if hyphenated != name && strings.HasPrefix(lower[start:], hyphenated) {
			return name, len(hyphenated)
		}
	}
	return "", 0
}
