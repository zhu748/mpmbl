...
func (s *Store) LastPolledAccountID() string {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.cfg.Runtime.LastPolledAccountID
}

func (s *Store) SetLastPolledAccountID(accountID string) error {
	return s.Update(func(c *Config) error {
		c.Runtime.LastPolledAccountID = accountID
		return nil
	})
}
