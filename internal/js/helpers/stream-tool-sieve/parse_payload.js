...
function scanToolMarkupTagAt(text, start) {
  ...
  let dsmlLike = false;
  const pipeEnd = consumeToolMarkupPipe(raw, i);
  if (pipeEnd >= 0) {
    dsmlLike = true;
    i = pipeEnd;
    let next = consumeToolMarkupSeparator(raw, i);
    while (next >= 0) {
      i = next;
      next = consumeToolMarkupSeparator(raw, i);
    }
  }
  const prefixEnd = consumeToolMarkupNamePrefix(lower, raw, i);
  if (prefixEnd >= 0) {
    dsmlLike = true;
    i = prefixEnd;
    let next = consumeToolMarkupSeparator(raw, i);
...
function findPartialToolMarkupStart(text) {
  ...
  const candidates = [
    '<tool_calls', '<tool-calls', '<invoke', '<parameter',
    '<|tool_calls', '<|tool-calls', '<|invoke', '<|parameter',
    '<｜tool_calls', '<｜tool-calls', '<｜invoke', '<｜parameter',
    '<|#tool_calls', '<|#tool-calls', '<|#invoke', '<|#parameter',
    '<|*tool_calls', '<|*tool-calls', '<|*invoke', '<|*parameter',
    '<dsml-tool_calls', '<dsml-tool-calls', '<dsml-invoke', '<dsml-parameter',
    '<dsml_tool_calls', '<dsml_tool-calls', '<dsml_invoke', '<dsml_parameter',
    '<```dsml tool_calls', '<```dsml tool-calls', '<```dsml invoke', '<```dsml parameter',
    '<|dsml|tool_calls', '<|dsml|tool-calls', '<|dsml|invoke', '<|dsml|parameter',
    '<｜dsml|tool_calls', '<｜dsml|tool-calls', '<｜dsml|invoke', '<｜dsml|parameter',
    '<dsmltool_calls', '<dsmltool-calls', '<dsmlinvoke', '<dsmlparameter',
    '<dsml tool_calls', '<dsml tool-calls', '<dsml invoke', '<dsml parameter',
    '<dsml|tool_calls', '<dsml|tool-calls', '<dsml|invoke', '<dsml|parameter',
    '<|dsmltool_calls', '<|dsmltool-calls', '<|dsmlinvoke', '<|dsmlparameter',
    '<|dsml tool_calls', '<|dsml tool-calls', '<|dsml invoke', '<|dsml parameter',
...
function consumeToolMarkupSeparator(text, idx) {
  if (idx >= text.length) {
    return -1;
  }
  ...
  return consumeToolMarkupPipe(text, idx);
}

function consumeToolMarkupNamePrefix(lower, text, idx) {
  const direct = consumeToolMarkupNamePrefixOnce(lower, text, idx);
  if (direct >= 0) {
    return direct;
  }
  return consumeToolMarkupBacktickPrefix(lower, text, idx);
}

function consumeToolMarkupNamePrefixOnce(lower, text, idx) {
  for (const prefix of ['dsml', 'dmsl']) {
    if (!lower.startsWith(prefix, idx)) {
      continue;
    }
    const next = idx + prefix.length;
    if (next >= text.length) {
      return next;
    }
    if (text[next] === '-' || text[next] === '_') {
      return next + 1;
    }
    return next;
  }
...
function consumeToolMarkupBacktickPrefix(lower, text, idx) {
  if (idx >= text.length || text[idx] !== '`') {
    return -1;
  }
  for (let next = idx; next < text.length && text[next] === '`'; next += 1) {
    const prefixEnd = consumeToolMarkupNamePrefixOnce(lower, text, next + 1);
    if (prefixEnd >= 0) {
      return prefixEnd;
    }
  }
...
function matchToolMarkupName(lower, start) {
  for (const name of TOOL_MARKUP_NAMES) {
    ...
    if (hyphenated !== name && lower.startsWith(hyphenated, start)) {
      return { name, len: hyphenated.length };
    }
  }
  return { name: '', len: 0 };
}
