...
func TestProcessToolSieveInterceptsDMSLToolCallVariantsWithoutLeak(t *testing.T) {
	cases := []struct {
		name   string
		chunks []string
	}{
		{
			name: "pipe marker split wrapper",
			chunks: []string{
				"<|DMSL|tool",
				"_calls>\n",
				`  <|DMSL|invoke name="read_file">` + "\n",
				`    <|DMSL|parameter name="path">README.MD</|DMSL|parameter>` + "\n",
				"  </|DMSL|invoke>\n",
				"</|DMSL|tool_calls>",
			},
		},
		{
			name: "no separator marker",
			chunks: []string{
				"<|dmsltool_calls>\n",
				`  <|dmslinvoke name="read_file">` + "\n",
				`    <|dmslparameter name="path">README.MD</|dmslparameter>` + "\n",
				"  </|dmslinvoke>\n",
				"</|dmsltool_calls>",
			},
		},
		{
			name: "space separator marker",
			chunks: []string{
				"<dmsl tool_calls>\n",
				`  <dmsl invoke name="read_file">` + "\n",
				`    <dmsl parameter name="path">README.MD</dmsl parameter>` + "\n",
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if strings.Contains(strings.ToLower(textContent), "dmsl") || strings.Contains(textContent, "read_file") {
				t.Fatalf("DMSL tool call content leaked to text: %q", textContent)
			}
			if toolCalls != 1 {
				t.Fatalf("expected one DMSL tool call, got %d events=%#v", toolCalls, events)
			}
		})
	}
}
