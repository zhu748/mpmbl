...
func TestPoolAcquireWaitWakesMultipleAvailableSlots(t *testing.T) {
	t.Setenv("DS2API_ACCOUNT_MAX_INFLIGHT", "1")
	t.Setenv("DS2API_ACCOUNT_MAX_QUEUE", "4")
	t.Setenv("DS2API_CONFIG_JSON", `{
		"keys":["k1"],
		"accounts":[
			{"email":"acc1@example.com","token":"token1"},
	...
	if !ok {
		t.Fatal("expected acc1 acquire success")
	}
	acc2, ok := pool.Acquire("acc2@example.com", nil)
	if !ok {
		t.Fatal("expected acc2 acquire success")
	}
	...
	for i := 0; i < 2; i++ {
		go func() {
			...
			defer cancel()
			acc, ok := pool.AcquireWait(ctx, "", nil)
			results <- result{id: acc.Identifier(), ok: ok}
		}()
	}

	waitForWaitingCount(t, pool, 2)
	...
	for i := 0; i < 2; i++ {
		...
		case res := <-results:
			if !res.ok {
				t.Fatalf("expected waiter %d to acquire account", i+1)
			}
			seen[res.id] = true
		case <-time.After(time.Second):
	...
	if !seen["acc1@example.com"] || !seen["acc2@example.com"] {
		t.Fatalf("expected both accounts to be reacquired, got %v", seen)
	}
}
